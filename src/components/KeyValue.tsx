...
export const KeyValue: React.FC<React.PropsWithChildren<KeyValueProps>> = ({
  ...
  const CardComponent = cardType === 'fact' ? TrueCard : cardType === 'falsehood' ? FalseCard : CompactCard;
  return (
    <CardComponent>
      <LeftRight>
...
export const NoKey: React.FC<React.PropsWithChildren<{}>> = ({ children }) => {
  // @ts-ignore
  return <KeyValue cardType={"falsehood"} value={<Falsehood />}> {children} </KeyValue>;
};

export const YesKey: React.FC<React.PropsWithChildren<{}>> = ({ children }) => {
  // @ts-ignore
  return <KeyValue cardType={"fact"} value={<Facts />}> {children} </KeyValue>;
};
